...
describe('the view vocabulary is a parameter', () => {
  test('a host carrying a further view declares it without forking the parser', () => {
    const result = makeRegistryParser({
      absolute: repoFor(REGISTRY),
      fixturesRelative: FIXTURES_RELATIVE,
      registryRelative: `${FIXTURES_RELATIVE}/corpus.ts`,
      ...
      listPageRelative: LIST_PAGE_RELATIVE,
      listRowsKey: 'purchases',
      views: ['detail', 'list', 'cable'],
    })();

    assert.deepEqual(result.views, ['detail', 'list', 'cable']);
    // `cell-map` exits 2 unless every view a cell carries appears in the vocabulary, so widening has
    // to leave the emitted cells reachable — which is the direction this parameter may move.
    assert.ok(result.cells.every(cell => result.views.includes(cell.view)));
  });
});
